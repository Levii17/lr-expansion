...
function buildArchitecturalModel() {
  ...
  addBox(
    ...
    [4.8, 0.16, 3.1],
    [0, 0.38, 0],
    makeMaterial(0x4f595a, 1, 0.9, 0.05),
  );
  const plinth = addBox(
    foundation,
    [4.4, 0.22, 2.7],
  ...
  [-1.95, -0.65, 0.65, 1.95].forEach((x) => {
    levels.forEach((y) =>
      addBox(structure, [0.095, 0.92, 0.095], [x, y, -1.18], frameMaterial),
    );
  });
  [-1.95, -0.65, 0.65, 1.95].forEach((x) => {
    levels.forEach((y) =>
  ...
  levels.forEach((y) =>
    addBox(structure, [4.1, 0.08, 0.09], [0, y - 0.4, -1.18], frameMaterial),
  );
  levels.forEach((y) =>
    addBox(structure, [4.1, 0.08, 0.09], [0, y - 0.4, 1.18], frameMaterial),
  );
  levels.forEach((y) =>
    addBox(structure, [0.09, 0.08, 2.45], [-1.95, y - 0.4, 0], frameMaterial),
  );
  levels.forEach((y) =>
    addBox(structure, [0.09, 0.08, 2.45], [1.95, y - 0.4, 0], frameMaterial),
  );
  ...
  [-1.95, 1.95].forEach((x) => {
    levels
      .slice(0, 4)
      .forEach((y) =>
        addBox(envelope, [0.035, 0.68, 0.66], [x, y, 0], sideGlass),
      );
  });
  addBox(
    envelope,
    [4.12, 0.055, 2.48],
    [0, 5.22, 0],
    makeMaterial(PAPER, 0.75, 0.35, 0.1),
  );
  model.add(envelope);
  ...
  [-1.95, 1.95].forEach((x) =>
    addBox(facade, [0.13, 4.42, 0.14], [x, 2.95, -1.3], facadeMaterial),
  );
  ...
  [-1.48, -0.49, 0.49, 1.48].forEach((x) => {
    levels
      .slice(0, 4)
      .forEach((y) =>
        addBox(facade, [0.63, 0.62, 0.08], [x, y, -1.34], darkGlass),
      );
  });
  ...
  addBeam(
    crane,
    new THREE.Vector3(0, 6.65, 0),
    new THREE.Vector3(3.35, 6.65, 0),
    0.055,
    detailMaterial,
  );
  addBeam(
    crane,
    new THREE.Vector3(2.7, 6.65, 0),
    new THREE.Vector3(2.7, 4.4, 0),
    0.028,
    detailMaterial,
  );
  detail.add(crane);
...
export function ConstructionScene({ activeStage }: ConstructionSceneProps) {
  ...
  useEffect(() => {
    ...
    revealStage(stageRef.current, true);
  ...
  return (
    <div
      className="lr-three-scene"
      ref={rootRef}
      data-stage={activeStage}
      aria-label="Three dimensional architectural build visualization"
    >
      <canvas ref={canvasRef} aria-hidden="true" />
      <div className="lr-three-scene-legend lr-mono">
        <span>THREE / STRUCTURAL MODEL</span>
        <span>STAGE {String(activeStage + 1).padStart(2, "0")} OF 05</span>
      </div>
    </div>
  );
}
